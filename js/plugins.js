		            ...
		            var infinityRunner = function() {
		                if (settings.url !== null) {
		                    if  (settings.force || (settings.triggerAt >= (settings.container.height() - el.height() - el.scrollTop()))) {
		                        ...
		                        req = $.get(settings.url, 'page='+settings.page+'&pages='+settings.pages+'&query='+settings.query+'&sid='+settings.sid+'&did='+settings.did, function(data) {
		                        	if (data !== '') {
		                                if (settings.page > 1) {
